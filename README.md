...
| [`finalize-project`](./skills/finalize-project/) | The ordered, language-agnostic (Python / MATLAB / R) checklist for making a project reproducible and publicly archived once the paper is accepted — a definition of done per item: Methods↔code audit in both directions (claims table, trace the *invoked* value, trust artifacts over code), paper-result→code map + fresh-clone smoke test, pinned envs with lock exports and in-house libraries pinned to the commit that produced the fits, OpenNeuro release from a staging copy with a defacing QC gate that looks at pixels (not a JSON flag) and a whole-tree identifier audit, figshare behavioural table + codebook, GitHub→Zenodo DOI, and the availability statements cross-linked everywhere. References include funder/institutional obligations with primary sources (SNSF, ERC, UZH, Swiss HRA) and two generic script templates (identifier audit, byte-level deface-apply + QC) in [`references/`](./skills/finalize-project/references/). Pairs with [`data-archival`](./skills/data-archival/) and [`cogneuro-project`](./skills/cogneuro-project/). |
